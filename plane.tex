...
\begin{proof}
  ...
  vertices. We want to apply \lemref{outerplanar-fancy} to the graph $B$
  to obtain an embedding of $B$ with $v_{j+1},\ldots,v_{r}$ below the
  segment $cd$.  The clustering provided Property~4 of the resulting
  embedding is unimportant to us, so we only use an index sequence of
  length $t=2$ chosen so that $w_{i_1}=v_{j+1}$ and $w_{i_2}=v_r$.
  ...
  $i_2,\ldots,i_{t-1}$ to be the set of all top vertices $v$ such that $G$
  contains a path $vxy$ such that $y$ is a bottom vertex and $x\not\in C$.


We do this by applying \lemref{outerplanar-fancy} with $G=B$, $v_1,\ldots,v_r=
