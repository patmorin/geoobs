...
\begin{proof}
   ...
   \begin{enumerate}
     ...
     \item No geodesic passes close to a vertex except its endpoints:
      ...
      $i\in\{0,\ldots,D\}$.  Then, as argued above $uw_i$ and $x$ are
      independent random bit strings, so $\Pr\{d_{Q_D}(uw_i,x)\} <
      (1-\eps)D/2)\} \le n^{-\Omega(c)}$.  By the union bound, the
      probbaility that there exists any triple of vertices $u,w,x$ and
      an index $i$ for which this is not true is also $n^{-\Omega(c)}$.
